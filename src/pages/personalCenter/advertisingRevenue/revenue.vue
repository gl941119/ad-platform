<template>
    <div class="advertising_revenue">
        <div class="advertising_revenue_top">
            ...
            <el-button class="advertising_revenue_top_money changeColor"
                       @click="withdraw()">{{$t('project.withdraw')}}</el-button>
        ...
        <div class="withdraw"
            ...
            <div class="withdraw_box">
                <span class="withdraw_box_back"
                      @click="withdraw">
                    <i class="el-icon-close"></i>
                </span>
                <ul class="withdraw_item">
                    <li class="withdraw_item_li">
                        <label>{{$t('project.useBalance')}}</label>
                        <span class="span">{{balance}}</span>
                    </li>
                    <li class="withdraw_item_li">
                        <label>{{$t('project.handlingFee')}}</label>
                        <span class="span">{{handlingFee}} AFDT</span>
                    </li>
                    <li class="withdraw_item_li">
                        <label>{{$t('project.arrival')}}</label>
                        <span class="span">{{arrival}} AFDT</span>
                    </li>
                    <li class="withdraw_item_li">
                        <span>{{$t('project.revenue')}}</span>
                        <i class="custom-element-icon-jiantou1-copy"></i>
                        <span>{{$t('project.myWallet')}}</span>
                    </li>
                    <li class="withdraw_item_li">
                        <label>{{$t('project.withdrawal')}}：</label>
                        <input class="tixian"
                               autocomplete="off"
                               :class="[errors.has('money')?'llo':'']"
                               :data-vv-as="$t('project.emptyMoney')"
                               v-validate="{required:true,regex:/^\d{5,}/}"
                               name="money"
                               v-on:change="getHandlingFee"
                               :placeholder="$t('project.enterMoney')"
                               v-model="money" />
                    </li>
                    <span class="is-danger"
                          v-show="errors.has('money')">{{ errors.first('money') }}</span>
                    <span class="is-danger"
                          v-if="insufficient">{{$t('accountFlow.insufficient')}}</span>
                    <li class="withdraw_item_li">
                        <label>{{$t('passwordInfo.tradePassword')}}：</label>
                        <input type="password"
                               style="display:none;" />
                        <!--<input class="tixian" type="text" onfocus="this.type='password'" />-->
                        <input onfocus="this.type='password'"
                               class="tixian"
                               :class="[errors.has('tradePassword')?'llo':'']"
                               :data-vv-as="$t('messageNotice.emptyTradePassword')"
                               v-validate
                               data-vv-rules="required"
                               name="tradePassword"
                               :placeholder="$t('passwordInfo.enterTradePassword')"
                               v-model="tradePassword" />
                    </li>
                    <span class="is-danger"
                          v-show="errors.has('tradePassword')">{{ errors.first('tradePassword') }}</span>
                    <div style="text-align: center;">
                        <button class="right_now"
                                @click="withdrawls">{{$t('project.withdraw')}}</button>
                    </div>
                </ul>
            </div>
        </div>
        ...
        <div class="advertising_revenu_account_flow">
            <div class="advertising_revenu_account_flow_time">
                ...
                <div class="advertising_revenu_account_flow_date">
                    <el-date-picker v-model="startTime"
                                    type="date"
                                    :placeholder="$t('accountFlow.startTime')">
                    </el-date-picker>
                    <el-date-picker v-model="endTime"
                                    type="date"
                                    :placeholder="$t('accountFlow.endTime')">
                    </el-date-picker>
                    <button class="search"
                            @click="search">{{$t('accountFlow.search')}}</button>
                </div>
            </div>
            <el-tabs type="border-card"
                ...
                <el-tab-pane value="1">
                    ...
                    <el-table :data="flowDatas"
                              style="width: 100%">
                        <el-table-column prop="createTime"
                                         width="150"
                                         :label="$t('accountFlow.dataTime')">
                        </el-table-column>
                        <el-table-column prop="desc"
                                         :label="$t('accountFlow.desc')">
                        </el-table-column>
                        ...
                        <el-table-column prop="money"
                                         width="200"
                                         :label="$t('accountFlow.amountOfMoney')">
                        </el-table-column>
                    </el-table>
